...
async def create_age_group(age_group: AgeGroupCreate):
    ...
    result = db.age_groups.insert_one(age_group_dict)
    created_age_group = db.age_groups.find_one({"_id": result.inserted_id})
    created_age_group["id"] = str(created_age_group.pop("_id"))
    return created_age_group

async def get_age_group(age_group_id: str):
    result = db.age_groups.find_one({"_id": ObjectId(age_group_id)})
    if result:
        result["id"] = str(result.pop("_id"))
    return result

async def get_all_age_groups():
    result = db.age_groups.find()
    age_groups = []
    for doc in result:
...
async def update_age_group(age_group_id: str, age_group: AgeGroupUpdate):
    result = db.age_groups.update_one(
        {"_id": ObjectId(age_group_id)}, 
        {"$set": age_group.model_dump()}
    )
    if result.modified_count > 0:
        updated = db.age_groups.find_one({"_id": ObjectId(age_group_id)})
        updated["id"] = str(updated.pop("_id"))
        return updated
    return None

async def delete_age_group(age_group_id: str):
    result = db.age_groups.delete_one({"_id": ObjectId(age_group_id)})
